...
aliases:
  - /blog/19-recipe-formats-compared/
---
...
## Common Recipe Formats Compared

There's no single standard for storing recipes digitally. Different formats emerged from different eras and needs — BBS archives, XML web services, SEO, and plain text editing — each trading off human readability against machine parseability.

**Plain text and Markdown** — the most common "format" for personal recipe collections. Easy to write, readable everywhere, syncs fine. The problem: there's no structure. A computer can't reliably extract "400g canned tomatoes" from a Markdown bullet list because everyone formats them differently. No automated shopping lists. No scaling. Just text.
...
**MealMaster (`.mmf`)** — a fixed-width columnar format from the late-1990s BBS era. Hundreds of thousands of legacy recipes exist in it, but the strict column layout makes it fragile to parse and there's no active development. Good for archives, not for new recipes.

**RecipeML (XML)** — a properly structured XML format from 2000, with separate quantity/unit/item fields. The ecosystem is dead (last release 2002) and the XML is extremely verbose — the same recipe that's 8 lines of Cooklang runs ~47 lines of RecipeML.

**Open Recipe Format (YAML)** — clean, readable, version-control friendly, with structured ingredient fields. But ingredients and steps are stored separately, so you lose the connection between "add garlic" in the steps and the garlic entry in the list. Small ecosystem.
...
The fundamental tension is between human readability and machine parseability: Markdown is the easiest to write but computers can't reliably extract structured data from it; JSON-LD is perfectly machine-readable but nobody writes it by hand. Cooklang sits in the middle — you write natural recipe text, and a few annotations make it machine-parseable without sacrificing readability.

If you're building software against recipe data and want a code-level view — parser availability, AST structure, what your code actually gets back from each format — see [Recipe Formats for Developers](/blog/41-recipe-formats-for-developers/).
...
This is the "fundamental tension" from the comparison above — plain text is human-readable but not machine-parseable; JSON is machine-readable but not human-writable. Cooklang is both, because the annotations stay out of the way when you're reading.
